...
## Architecture
- **State management**: App, Layout, Toast, and Roster contexts (Context + `useReducer`).
...
## Key Utilities + Hooks
- `utils/`: abilities, array, datasheet, faction, keywords, paths, roster, wargear helpers
...
- All tests should wrap renders with `TestWrapper` (`src/test/test-utils.tsx`) to provide Router + contexts.
