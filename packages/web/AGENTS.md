...
- `/faction/:factionSlug/detachment/:detachmentSlug` - Detachment rules (abilities, enhancements, stratagems)
- `/collections` - Collections list (create via bottom sheet)
...
- `/rosters` - Saved roster list (create via bottom sheet)
...
- `@depot/core/utils/{datasheets,wargear,abilities,roster,roster-legality,rebind,collection,detachments,model-costs}`: canonical normalization + data builders used by hooks/routes.
- `src/utils/`: UI-specific wrappers only (e.g., ability tag styling, `buildAbsoluteUrl` which reads browser env).
- `hooks/`: async, bookmarks, collections/rosters loaders, datasheet/faction loaders, debounce, document title, media query, persisted tag selection, roster unit selection, scroll-to-hash, select, share action.
